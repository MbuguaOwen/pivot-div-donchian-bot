...
def update_stop(state: StopState, atr: float, params: StructureAtrTrailParams) -> float:
    ...
    spacing_r = params.trail_trigger_r - params.lock_r
    ...
    if (not state.trailing_active) and best_r >= params.trail_trigger_r:
        if state.side == "LONG":
            lock_sl = state.entry + params.lock_r * state.R
